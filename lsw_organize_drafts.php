...
final class LSW_Organize_Drafts {
	...
	public function __construct() {
		...
		LSW_Organize_Drafts::$draft_types = array ('Old Junk', 'Idea',
			'Outline',
			'Needs Images', 'Completed');
		$this->hooks();
	...
	private function hooks() {
		...
		add_action('add_meta_boxes', array('LSW_Organize_Drafts','update_meta_boxes'), 20, 2 );
		...
		foreach($post_types as $type) {
			add_filter( 'manage_edit-' . $type . '_sortable_columns',  array('LSW_Organize_Drafts',
	...
	public static function save_taxonomy_data($post_id, $post) {
		...
		if(isset($_POST['post_lswdrafttype'])) {
			$post_type = filter_var($_POST['post_lswdrafttype'], FILTER_SANITIZE_STRING);
			wp_set_object_terms( $post_id, $post_type, 'lswdrafttype' );
		}
	}


	public static function update_meta_boxes($post_type, $post) {
	...
	public function setup_taxonomy() {
			...
			$test = get_terms( 'lswdrafttype', 'hide_empty=0');
